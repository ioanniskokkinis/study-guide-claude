...
export function MasterySparkline({ trend }: { trend: WeeklyMasteryPoint[] }) {
  ...
  if (knownWeeks.length < 2) {
    return <p className="text-sm text-zinc-500">Not enough history yet to show a trend — check back after a few study sessions.</p>;
  }
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="h-16 w-full" preserveAspectRatio="none" role="img" aria-label="Mastery trend over recent weeks">
        <path d={path} fill="none" stroke="currentColor" strokeWidth={2} className="text-zinc-900 dark:text-zinc-50" />
      </svg>
